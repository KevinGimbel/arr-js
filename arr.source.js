 ...
 *
 * @params {Array} - Any number of Array(s) that should be merged into the existing Array
 */

Array.prototype.concat = function() {
  var dump = this;
  
  // If there's no argument just return the element.
  if(arguments.length === 0) {
    return this;
  } 
  for(key in arguments) {
    // Check if the arguments constructor is an Array.
    if(arguments[key].constructor.toString().indexOf('Array') > -1) {
      console.log('Array found: ' + arguments[key]);
      // however, if it's an array call the Array.merge() function on it, passing the
      // current key as an argument.
      Array.prototype.merge.call(dump, arguments[key]);
    } else {
      // otherwise, just push the argument into the new array.
      dump.push(arguments[key]);
    }
  }
    // and finally, return the concatinated array!
    return dump;
}
...
Array.prototype.concatLazy = function() {
  var array = this;
  // just append all arguments, no matter what they are.
  for(key in arguments) {
